    ...
    public function authenticate()
{
        ...
        if ($user) {
            ...
            if ($user->auth_provider === 'local') {
                ...
                if ($this->email->send()) {
                    ...
                    return redirect()->to('/forgot_password')->with('success', 'OTP sent to your email.');
                } else {
    ...
    {
        $otp = $this->request->getPost('otp'); // Get OTP from the form input
        $user = $this->userModel->verifyOtp($otp); // Validate OTP against the database

        if ($user) {
            session()->set('reset_user_id', $user['user_id']); // Use array syntax
            session()->remove('otp_sent'); // Clear OTP-related session data
    ...
    public function resetPasswordForm()
    {
        ...
        return view('reset_password_form'); // Load Reset Password form
    }
