    ...
    public function getAttendanceBySubject(?int $userId = null, bool $isAdmin = false): array
{
    if (!$isAdmin) {
        ...
        $query = $this->select('courses.course_name, classes.subject_code, attendance.class_id, COUNT(attendance.attendance_id) as total')
...
//         return $this->select('courses.course_name, classes.subject_code, COUNT(attendance.attendance_id) as total')
//             ->join('classes', 'classes.class_id = attendance.class_id') // Join classes with attendance
//             ->join('courses', 'courses.course_code = classes.subject_code') // Join courses with classes
//             ->where('classes.user_id', $userId) // Filter by lecturer ID
//             ->groupBy('courses.course_name, classes.subject_code') // ✅ Group by course only, not class
//             ->orderBy('classes.subject_code', 'ASC')
//             ->findAll();
//     }

//     // Admin Query: Fetch all courses' attendance
//     return $this->select('courses.course_name, classes.subject_code, COUNT(attendance.attendance_id) as total')
//         ->join('classes', 'classes.class_id = attendance.class_id') // Join classes with attendance
//         ->join('courses', 'courses.course_code = classes.subject_code') // Join courses with classes
//         ->groupBy('courses.course_name, classes.subject_code') // ✅ Group by course only
//         ->orderBy('classes.subject_code', 'ASC')
//         ->findAll();
// }
